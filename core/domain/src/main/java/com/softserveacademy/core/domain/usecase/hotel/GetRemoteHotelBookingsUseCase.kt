...
/**
 * Use case for retrieving all hotel bookings from the remote server.
 */
class GetRemoteHotelBookingsUseCase @Inject constructor(
    private val repository: HotelBookingRepository
) {
    suspend operator fun invoke(): AppResult<List<HotelBooking>> {
        return repository.getRemoteBookings()
    }
}
